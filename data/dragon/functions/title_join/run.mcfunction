...
execute at @e[type=area_effect_cloud,tag=drg_player,limit=1,sort=nearest] as @p[tag=drg_title,distance=..1,sort=nearest] run title @s times 0 80 20
execute at @e[type=area_effect_cloud,tag=drg_player,limit=1,sort=nearest] as @p[tag=drg_title,distance=..1,sort=nearest] run title @s title {"text": "W","color": "green","bold": true}
